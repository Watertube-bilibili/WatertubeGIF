...
function createStream(initialSize = 512) {
  ...
  function ensure(extra) {
    ...
    let nextLength = bytes.length;
    while (nextLength < needed) {
      nextLength *= 2;
    }
    let next = new Uint8Array(nextLength);
    next.set(bytes);
    bytes = next;
  }
  ...
  return {
    ...
    writeBytesView(view, start, count) {
      ensure(count);
      bytes.set(view.subarray(start, start + count), length);
      length += count;
    },
    bytesView() {
